...
from backend.scoring.batch_scoring import batch_offline_scores, grade_batch
...
def test_batch_offline_scores_maps_similarity_to_percent_clamped_rounded():
    ...
    with patch("backend.scoring.batch_scoring.batch_similarities", return_value=sims) as mock_sim:
...
def test_batch_offline_scores_empty_answers_returns_empty():
    ...
    with patch("backend.scoring.batch_scoring.batch_similarities") as mock_sim:
...
def test_grade_batch_force_online_all_success():
    ...
    with patch("backend.scoring.batch_scoring.online_score", side_effect=[80.0, 90.0, 70.0]) as mock_online, \
         patch("backend.scoring.batch_scoring.offline_score") as mock_offline:
...
def test_grade_batch_force_online_partial_failure_degrades():
    ...
    with patch("backend.scoring.batch_scoring.online_score", side_effect=[80.0, None, None]), \
         patch("backend.scoring.batch_scoring.offline_score", side_effect=[55.0, 60.0]):
        results = grade_batch("参考", ["a", "b", "c"], force_online=True)
    assert results == [
        {"score": 80.0, "method": "online", "degraded": False, "routed": False},
...
def test_grade_batch_auto_no_route_returns_offline():
    ...
    with patch("backend.scoring.batch_scoring.batch_offline_scores", return_value=[50.0, 60.0]), \
         patch("backend.scoring.batch_scoring.should_route", return_value=False) as mock_route, \
         patch("backend.scoring.batch_scoring.online_score") as mock_online:
...
def test_grade_batch_auto_route_success_returns_online():
    ...
    with patch("backend.scoring.batch_scoring.batch_offline_scores", return_value=[40.0]), \
         patch("backend.scoring.batch_scoring.should_route", return_value=True), \
         patch("backend.scoring.batch_scoring.online_score", return_value=90.0):
...
def test_grade_batch_auto_route_failure_degrades_to_offline():
    ...
    with patch("backend.scoring.batch_scoring.batch_offline_scores", return_value=[40.0]), \
         patch("backend.scoring.batch_scoring.should_route", return_value=True), \
         patch("backend.scoring.batch_scoring.online_score", return_value=None):
        results = grade_batch("参考", ["a"], force_online=False)
...
def test_grade_batch_results_length_matches_answers():
    ...
    with patch("backend.scoring.batch_scoring.batch_offline_scores", return_value=[10.0, 20.0, 30.0]), \
         patch("backend.scoring.batch_scoring.should_route", return_value=True), \
         patch("backend.scoring.batch_scoring.online_score", return_value=99.0):
...
def test_grade_batch_quality_mode_fast_keeps_75_offline():
    ...
    with patch("backend.scoring.batch_scoring.batch_offline_scores", return_value=[75.0]), \
         patch("backend.scoring.batch_scoring.online_score", return_value=90.0):
...
def test_grade_batch_unknown_quality_mode_raises_even_force_online():
    ...
    with patch("backend.scoring.batch_scoring.online_score", return_value=90.0), \
         patch("backend.scoring.batch_scoring.offline_score", return_value=50.0):
