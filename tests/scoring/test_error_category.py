...
from backend.scoring.error_category import (
    CATEGORY_CONCEPT,
    CATEGORY_MASTER,
    CATEGORY_MOSTLY,
    CATEGORY_NONE,
...
def test_classify_error_default_ai_off_returns_rule_and_empty_reason():
    ...
    with patch("backend.scoring.error_category.classify_ai") as mock_ai:
...
def test_classify_error_ai_mode_outside_band_returns_rule(monkeypatch):
    ...
    monkeypatch.setattr("backend.core.config.ERROR_AI_BAND_LOW", 30.0)
    monkeypatch.setattr("backend.core.config.ERROR_AI_BAND_HIGH", 85.0)
    with patch("backend.scoring.error_category.classify_ai") as mock_ai:
...
def test_classify_error_ai_mode_in_band_passes_through(monkeypatch):
    ...
    monkeypatch.setattr("backend.core.config.ERROR_AI_BAND_LOW", 30.0)
    monkeypatch.setattr("backend.core.config.ERROR_AI_BAND_HIGH", 85.0)
    with patch("backend.scoring.error_category.classify_ai", return_value=("计算错误", "符号算错")) as mock_ai:
...
def test_classify_error_ai_mode_ai_failure_degrades_to_rule(monkeypatch):
    ...
    monkeypatch.setattr("backend.core.config.ERROR_AI_BAND_LOW", 30.0)
    monkeypatch.setattr("backend.core.config.ERROR_AI_BAND_HIGH", 85.0)
    with patch("backend.scoring.error_category.classify_ai", side_effect=RuntimeError("boom")):
...
def test_classify_ai_parses_valid_json():
    ...
    with patch("backend.scoring.deepseek._get_client", return_value=client) as mock_get:
...
def test_classify_ai_category_not_in_table_raises_value_error():
    ...
    with patch("backend.scoring.deepseek._get_client", return_value=client):
