...
from backend.scoring.embedding import _BASE, _post, encode, semantic_similarity
...
def test_encode_parses_vectors_to_float32_ndarray():
    ...
    with patch("backend.scoring.embedding._post", return_value={"vectors": [[0.5, 0.5], [0.2, 0.8]]}):
        result = encode(["甲", "乙"])
...
def test_encode_missing_vectors_raises_runtime_error():
    ...
    with patch("backend.scoring.embedding._post", return_value={}):
...
def test_semantic_similarity_parses_score():
    ...
    with patch("backend.scoring.embedding._post", return_value={"score": 0.75}):
        assert semantic_similarity("参考", "答案") == 0.75
    with patch("backend.scoring.embedding._post", return_value={"score": 1}):
...
def test_post_network_error_raises_runtime_error_with_url_and_hint():
    ...
    with patch("backend.scoring.embedding._get_client", return_value=client):
        with pytest.raises(RuntimeError) as exc_info:
            _post("/encode", {"texts": ["甲"]})
    assert f"{_BASE}/encode" in str(exc_info.value)
    assert "请先启动 python -m servers.embedding.server" in str(exc_info.value)
...
def test_encode_network_error_raises_runtime_error_with_hint():
    ...
    with patch("backend.scoring.embedding._get_client", return_value=client):
        with pytest.raises(RuntimeError, match="请先启动 python -m servers.embedding.server"):
            encode(["甲"])
...
def test_offline_score_still_maps_mocked_semantic_similarity():
    """调用方回归：backend.scoring.engine.offline_score 经 mock semantic_similarity 仍正常映射百分制。"""
    from backend.scoring.engine import offline_score

    with patch("backend.scoring.engine.semantic_similarity", return_value=0.75) as mock_sim:
