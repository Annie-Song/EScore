...
from backend.scoring.embedding import _BASE, batch_similarities, encode_reference
...
def test_encode_reference_parses_vector():
    ...
    with patch("backend.scoring.embedding._post", return_value={"vector": [0.3, 0.7]}):
...
def test_encode_reference_empty_text_short_circuits_without_http():
    ...
    with patch("backend.scoring.embedding._post") as mock_post:
...
def test_batch_similarities_parses_scores_equal_length():
    ...
    answers = ["答1", "答2", "答3"]
    with patch("backend.scoring.embedding._post", return_value={"scores": [0.9, 0.1, 0.5]}):
...
def test_batch_offline_scores_still_maps_mocked_batch_similarities():
    ...
    from backend.scoring.batch_scoring import batch_offline_scores
