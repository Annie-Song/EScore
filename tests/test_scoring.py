...
from backend.scoring.engine import offline_score, online_score, grade_answer


def test_offline_score_maps_semantic_similarity_to_percent():
    with patch("backend.scoring.engine.semantic_similarity", return_value=0.75):
        assert offline_score("ref", "ans") == 75.0


def test_offline_score_clamps_negative_similarity_to_zero():
    with patch("backend.scoring.engine.semantic_similarity", return_value=-0.2):
...
def test_grade_answer_force_online_failure_falls_back_to_offline():
    with patch("backend.scoring.engine.get_points", return_value=None), \
         patch("backend.scoring.engine.semantic_similarity", return_value=1.0):
...
def test_grade_answer_allow_online_false_force_online_true_degrades_offline_no_deepseek():
    ...
    """
    with patch("backend.scoring.engine.semantic_similarity", return_value=0.5), \
         patch("backend.scoring.engine.get_points") as mock_points:
...
def test_grade_answer_auto_no_route_uses_offline_result():
    with patch("backend.scoring.engine.should_route", return_value=False), \
         patch("backend.scoring.engine.semantic_similarity", return_value=0.5):
        result = grade_answer("参考", "答案", False)
        assert result == {"score": 50.0, "method": "offline", "degraded": False, "routed": False}


def test_grade_answer_auto_route_success_returns_online_result():
    with patch("backend.scoring.engine.should_route", return_value=True), \
         patch("backend.scoring.engine.semantic_similarity", return_value=0.4), \
         patch("backend.scoring.engine.get_points", return_value=0.9):
        result = grade_answer("参考", "答案", False)
...
def test_grade_answer_auto_route_failure_falls_back_to_offline():
    with patch("backend.scoring.engine.should_route", return_value=True), \
         patch("backend.scoring.engine.semantic_similarity", return_value=0.4), \
         patch("backend.scoring.engine.get_points", return_value=None):
        result = grade_answer("参考", "答案", False)
...
def test_grade_answer_quality_mode_fast_keeps_75_offline():
    ...
    with patch("backend.scoring.engine.offline_score", return_value=75.0), \
         patch("backend.scoring.engine.get_points", return_value=0.9):
...
def test_grade_answer_unknown_quality_mode_raises_even_force_online():
    ...
    with patch("backend.scoring.engine.get_points", return_value=1.0), \
         patch("backend.scoring.engine.offline_score", return_value=50.0):
