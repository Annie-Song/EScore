...
通过 mock servers.ocr.core / servers.ocr.segment / servers.ocr.region 模块命名空间内的
...
from servers.ocr.server import app
...
def test_recognize_texts_returns_texts():
    ...
    with _make_client() as client, patch(
        "servers.ocr.core.recognize_texts", return_value=["第一题答案", "第二题答案"]
    ) as mock_rec:
        resp = client.post(
...
def test_recognize_texts_empty_paths_short_circuits_without_ocr():
    ...
    with _make_client() as client, patch("servers.ocr.core.recognize_texts") as mock_rec:
...
def test_recognize_texts_mock_raises_returns_500():
    ...
    with _make_client(raise_server_exceptions=False) as client, patch(
        "servers.ocr.core.recognize_texts", side_effect=RuntimeError("mock ocr fail")
    ):
        resp = client.post(
...
def test_segment_image_bbox_tuple_serialized_as_list():
    ...
    with _make_client() as client, patch(
        "servers.ocr.segment.segment_image", return_value=regions
    ) as mock_seg:
...
def test_segment_image_mock_raises_returns_500():
    ...
    with _make_client(raise_server_exceptions=False) as client, patch(
        "servers.ocr.segment.segment_image", side_effect=ValueError("mock segment fail")
    ):
        resp = client.post("/segment_image", json={"path": "hw.png"})
    assert resp.status_code == 500
...
def test_crop_region_bbox_converted_to_tuple_and_returns_dst():
    ...
    with _make_client() as client, patch(
        "servers.ocr.segment.crop_region", return_value="out.png"
    ) as mock_crop:
        resp = client.post(
            "/crop_region",
...
def test_regions_shared_enhance_returns_regions():
    ...
    with _make_client() as client, patch(
        "servers.ocr.region.regions_with_shared_enhance", return_value=expected
    ) as mock_fn:
        resp = client.post(
...
def test_regions_shared_enhance_none_degrades_to_null():
    ...
    with _make_client() as client, patch(
        "servers.ocr.region.regions_with_shared_enhance", return_value=None
    ) as mock_fn:
        resp = client.post(
            "/regions_with_shared_enhance",
