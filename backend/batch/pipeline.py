...
from backend.batch.task_store import update_task
...
def _regions_of(path: str, lang: str, enable_segment: bool) -> list[dict]:
    ...
    """
    if enable_segment:
        from backend.ocr.client import crop_region, segment_image

        regions = segment_image(path)
        if len(regions) > 1:
            from backend.ocr.client import regions_with_shared_enhance
...
def _scores_for(reference_text: str, work_texts: list[str], quality_mode: str) -> list[dict]:
    ...
    from backend.scoring.batch_scoring import grade_batch
...
def _classify_for(
    ...
    from backend.scoring.error_category import classify_error
