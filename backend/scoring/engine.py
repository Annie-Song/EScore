...
from backend.scoring.deepseek import get_points
from backend.scoring.embedding import semantic_similarity
...
def should_route(offline_score: float, routing: RoutingConfig | None = None) -> bool:
    ...
    测试可 patch backend.scoring.engine.ROUTING_MODE 生效；显式传入 routing 则按给定配置决策
    （双档预设与扫描工具复用）。
    """
    if routing is None:
