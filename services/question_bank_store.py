...
from services.question_bank_schema import ensure_schema
from utils import config

_FACET_SQL = (
    "SELECT {column} AS value, COUNT(*) AS count "
    "FROM question_bank{where} GROUP BY {column} ORDER BY count DESC"
)
...
class QuestionBankStore:
    ...
    def _school_clause(
        self, visible_school_id: str | None
    ) -> tuple[str, list]:
        """构造学校可见范围子句：None 仅全局，有值则全局加本校。"""
        if visible_school_id:
            return "(school_id IS NULL OR school_id=?)", [visible_school_id]
    ...
    def _filter_clause(
        ...
        visible_school_id: str | None = None,
    ) -> tuple[str, list]:
        ...
        school_clause, school_params = self._school_clause(visible_school_id)
    ...
    def facets(self, visible_school_id: str | None = None) -> dict:
        """返回各维度取值计数：subjects/qtypes/difficulties/source_types。"""
        school_clause, params = self._school_clause(visible_school_id)
        where = f" WHERE {school_clause}"
        ...
        with self._session() as conn:
            for name, column in (
                ...
                rows = conn.execute(
                    _FACET_SQL.format(column=column, where=where), params
                ).fetchall()
                result[name] = [
